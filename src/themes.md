---
layout: base
title: Themes

eleventyNavigation:
  parent: Styles
  key: Themes
---

# Themes

```md
- [ ] TODO: Describe usage methodology for designers
```

---

Some Rahvusarhiiv digital products use different gradient theme for key background elements throughout the user interface. These are built-in into the system.

<div class="bg-theme-ais flex items-center justify-center text-sm w-52 h-36 rounded shadow-sm mb-20">AIS Theme</div>

```html
<div class="bg-theme-ais">AIS Theme</div>
```
