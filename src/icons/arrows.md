---
layout: base
title: Nooltega ikoonid
relative: ../..

eleventyNavigation:
  parent: Icons
  key: Nooltega ikoonid
  order: 4
---

# Erinevate nooltega ikoonid

<div class="grid grid-cols-[repeat(auto-fill,minmax(8rem,1fr))] gap-x-6 gap-y-8 pt-6">

{% icon 'liiguta ülespoole' %}
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M4 12L5.41 13.41L11 7.83V20H13V7.83L18.58 13.42L20 12L12 4L4 12Z" fill="#323232"/>
</svg>
{% endicon %}

{% icon 'liiguta allapoole' %}
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M20 12L18.59 10.59L13 16.17V4H11V16.17L5.42 10.58L4 12L12 20L20 12Z" fill="#323232"/>
</svg>
{% endicon %}

{% icon 'värskenda/taasta' %}
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M17.65 6.35C16.2 4.9 14.21 4 12 4C7.58001 4 4.01001 7.58 4.01001 12C4.01001 16.42 7.58001 20 12 20C15.73 20 18.84 17.45 19.73 14H17.65C16.83 16.33 14.61 18 12 18C8.69001 18 6.00001 15.31 6.00001 12C6.00001 8.69 8.69001 6 12 6C13.66 6 15.14 6.69 16.22 7.78L13 11H20V4L17.65 6.35Z" fill="#323232"/>
</svg>
{% endicon %}

{% icon 'ava kõik' %}
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M12 5.83L15.17 9L16.58 7.59L12 3L7.40997 7.59L8.82997 9L12 5.83ZM12 18.17L8.82997 15L7.41997 16.41L12 21L16.59 16.41L15.17 15L12 18.17Z" fill="#323232"/>
</svg>
{% endicon %}

{% icon 'sulge kõik' %}
<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M7.40997 18.59L8.82997 20L12 16.83L15.17 20L16.58 18.59L12 14L7.40997 18.59ZM16.59 5.41L15.17 4L12 7.17L8.82997 4L7.40997 5.41L12 10L16.59 5.41Z" fill="#323232"/>
</svg>
{% endicon %}

<!-- mdi: keyboard_arrow_left  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z"/></svg>
{% endicon %}

<!-- mdi: keyboard_arrow_right  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41z"/></svg>
{% endicon %}

<!-- mdi: keyboard_arrow_up  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M7.41 15.41 12 10.83l4.59 4.58L18 14l-6-6-6 6 1.41 1.41z"/></svg>
{% endicon %}

<!-- mdi: keyboard_arrow_down  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M7.41 8.59 12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg>
{% endicon %}

<!-- mdi: keyboard_double_arrow_left  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
  <g><g><polygon points="17.59,18 19,16.59 14.42,12 19,7.41 17.59,6 11.59,12"/><polygon points="11,18 12.41,16.59 7.83,12 12.41,7.41 11,6 5,12"/></g></g>
</svg>
{% endicon %}

<!-- mdi: keyboard_double_arrow_right  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
  <g><g><polygon points="6.41,6 5,7.41 9.58,12 5,16.59 6.41,18 12.41,12"/><polygon points="13,6 11.59,7.41 16.17,12 11.59,16.59 13,18 19,12"/></g></g>
</svg>
{% endicon %}

<!-- mdi: keyboard_double_arrow_up  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
  <g><g><polygon points="6,17.59 7.41,19 12,14.42 16.59,19 18,17.59 12,11.59"/><polygon points="6,11 7.41,12.41 12,7.83 16.59,12.41 18,11 12,5"/></g></g>
</svg>
{% endicon %}

<!-- mdi: keyboard_double_arrow_down  -->
{% icon '' %}
<svg class="h-6 w-6" viewBox="0 0 24 24" fill="currentColor">
  <g><g><polygon points="18,6.41 16.59,5 12,9.58 7.41,5 6,6.41 12,12.41"/><polygon points="18,13 16.59,11.59 12,16.17 7.41,11.59 6,13 12,19"/></g></g>
</svg>
{% endicon %}

</div>
